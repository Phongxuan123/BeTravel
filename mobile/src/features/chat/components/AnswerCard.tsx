      ...
      {answer.sources.map((s) => (
        <View key={s.url} className="mt-3 rounded-lg border border-line bg-[#F4F8FF] p-3.5">
          ...
          <View className="mt-3 flex-row" style={{ gap: 10 }}>
            ...
            <Pressable className="h-11 flex-1 items-center justify-center rounded-md border border-line bg-surface">
...
function FeedbackRow({ feedback, onFeedback, showReport }: { feedback?: 'up' | 'down'; onFeedback: (v: 'up' | 'down') => void; showReport: boolean }) {
  return (
    <View className="mt-4 flex-row items-center justify-between">
      ...
      {showReport && (
        <Pressable className="flex-row items-center rounded-md border border-danger-line bg-surface px-3 py-2.5" style={{ gap: 6 }}>
